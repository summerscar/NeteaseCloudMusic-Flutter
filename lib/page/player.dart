...
class _PlayerPageState extends State<StatefulWidget> {
  ...
  StateModel state = context.watch<StateModel>();
    if (state.currentSong == null) {
      return Scaffold(
        body: Center(
          child: IconButton(icon: Icon(Icons.arrow_back_ios), onPressed: () => Navigator.pushNamed(context, '/'))),
        );
    }
    return Scaffold(
      body: Container(
        decoration: state.currentSongPic != null ? BoxDecoration(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Container(
                        child: state.player.builderCurrentPosition(
                          builder: (context, Duration duration) {
                            String nowTimeStr = '${duration.inMinutes.remainder(60)}:${duration.inSeconds.remainder(60).toString().padLeft(2, '0')}';
                            ...
                            return SliderTheme(
                              ...
                              child: Column(
                                children: [
                                  Slider(
                                    value: isChanged ? changedVal : duration.inSeconds.toDouble(),
                                    min: 0,
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        state.player.builderLoopMode(builder: (context, LoopMode mode) {
                          print('loop mode $mode');

                          return IconButton(
                            icon: Icon(mode == LoopMode.single
                            ? Icons.repeat_one
                            : (mode == LoopMode.playlist ? Icons.repeat : Icons.arrow_right_alt)),
                            color: Colors.white,
                            iconSize: 30,
